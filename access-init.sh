...
for email in ${GWBACKUPY_ACCOUNT_EMAILS}; do
  for service in ${GWBACKUPY_SERVICES}; do
    ...
    CMD_CHECK="python -m gwbackupy $GWBACKUPY_MAIN_OPTIONS $service access-check --email=$email"
    echo "Run: $CMD_CHECK"
    if ! $CMD_CHECK; then
      echo "$email account auth check failed for $service service"
      CMD_INIT="python -m gwbackupy $GWBACKUPY_MAIN_OPTIONS $service access-init --email=$email"
      echo "Run: $CMD_INIT"
      if ! $CMD_INIT; then
